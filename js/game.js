function init(){
	...
	var game = {
		...
		buttons: [
			...
			new Button('  Parar', '#fff', 200, 100, () => player.stand()),
			new Button('  Empezar', '#fff', 935, -430, () => game.go()),
			new Button('Asegurar', '#fff', 100, -80, () => player.insure()),
			//new Button('Split', '#fff', 100, -40, () => l('split')),
			new Button('Duplicar', '#fff', 100, -40, () => player.double()),
		...
		_alert: function(msg){
			var alertText = new createjs.Text(msg.msg, '25px Arial', 'orange');
		...
		startScreen: function(){
			...
			if(localStorage.getItem('BlackJackJs-userName')){
				...
				var titleText = new createjs.Text('BlackJack 21', '60px Arial', '#fff');
				...
				nameInput.x = 400;
				nameInput.y = 360;
				nameInput._visiblePostCursorText.text = 'NOMBRE';

				var submitText = new createjs.Text('ENTRAR', '20px Arial', '#fff');
				submitText.x = 608;
				submitText.y = 370;
				...
				submitText.addEventListener('click', function(event){
					player.name.value = nameInput._visiblePreCursorText.text || 'Jugador 1';
